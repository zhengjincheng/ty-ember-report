...
(function ($) {
	...
	function OverviewBar($el,options){
		...
		this._root				= options.root||"/assets";
	}
	...
	OverviewBar.prototype._caculateRatio=function(r){
			...
			if(this._opts.canvasWidth){
				if(isNaN(this._opts.canvasWidth) && this._opts.canvasWidth.indexOf("%")>0){
			...
			if(this._opts.canvasHeight){
				if(isNaN(this._opts.canvasHeight) && this._opts.canvasHeight.indexOf("%")>0){
